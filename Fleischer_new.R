...
testinp <- train_data[,train_metadata$Sample[train_metadata$Disease == "Normal"]]
rownames(testinp) <- train_data[,1]

dds <- DESeqDataSetFromMatrix(countData = as.matrix(testinp),
                              colData = data.frame(Sample = colnames(testinp),
                                                   Age = sapply(train_metadata$Age_Y[train_metadata$Disease == "Normal"],h)
                              ),
                              design = ~ 1)
...
trainSamps <- fleischersamps
testSamps <- fleischersamps
...
mod_var_imp <- as.data.frame(h2o.varimp(model))
...
aslpreds <- merge(as.data.frame(colData(dds)),aslpreds,by.x= "Sample", by.y="SampleID")
aslpreds$Age.x <- sapply(aslpreds$Age.x, h_inv)
aslpreds$Agediff <- aslpreds$Age.y - aslpreds$Age.x
