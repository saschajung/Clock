...
plot_ly(x = mat.pca$v[,1], y = mat.pca$v[,2],type = "scatter",color = colData(dds)$Batch) %>% layout(showlegend = F)
mat <- mat_ruv
...
inp_sva_test <- t(mat[,testSamps,drop = F])
...
inp_comp_asl <- as.data.frame(cbind(inp_sva_test[,genesInLevel,drop = F]))
inp_comp_asl_h2o <- as.h2o(inp_comp_asl)

preds <- as.data.frame(h2o.predict(model,inp_comp_asl_h2o))
aslpreds <- preds
aslpreds$predict <- sapply(aslpreds$predict, h_inv)
aslpreds<- data.frame(cbind(SampleID=rownames(inp_comp_asl),Age=as.numeric(aslpreds$predict)), stringsAsFactors=FALSE)
...
write.table(coeff,file = "./LIBERR137716_Coefficients_final.txt", sep = "\t", quote = F, row.names = F, col.names = T)
write.table(preds_total,file = "./LIBERR137716_Preds_final.txt", sep = "\t", quote = F, row.names = F, col.names = T)
