...
model <- h2o.glm(y = "Age_scaled",
                 ...
                 alpha = 0,
                 seed = 100,
                 max_active_predictors = ncol(inp_comp_level_h2o)-2,
                 solver = "IRLSM"
)
model
...
write.table(coeff,file = "./LIB052667_Coefficients_final.txt", sep = "\t", quote = F, row.names = F, col.names = T)
write.table(preds_total,file = "./LIBS052667_Preds_final.txt", sep = "\t", quote = F, row.names = F, col.names = T)
